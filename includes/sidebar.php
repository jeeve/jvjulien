            <div class="col-xs-6 col-sm-3 sidebar-offcanvas" id="sidebar" role="navigation">
               <ul class="nav" style="text-align: center">
                  <li><a href="/sommaire/sommaire.php">en route...</a></li>
                  <li><a href="/astronomie/astronomie.php">vers les étoiles</a></li>
                  <li><a href="/mer/mer.php">parfum salé</a>
					<ul style="display: none;">
						<li><a href="/mer/rivages.php">rivages</a></li>
						<li><a href="/mer/diapo.php">lumières marines</a></li>
						<li><a href="/mer/saint-jacut.php">Saint-Jacut de la Mer</a></li>
						<li><a href="/mer/safaga.php">Safaga</a></li>						
					</ul>	
				  </li>					
                  <li><a href="/loire/loire.php">au fil de l'eau</a>
					<ul style="display: none;">
						<li><a href="/loire/diapo.php">ainsi coule la Loire</a></li>
					</ul>	
				  </li>	                  
				  <li><a href="/foret/foret.php">peuple végétal</a>
					<ul style="display: none;">
						<li><a href="/foret/diapo.php">rencontres végétales</a></li>
					</ul>	
				  </li>	 
                  <li><a href="/impressions/impressions.php">impressions</a></li>
                  <li><a href="/sensations/sensations.php">sensations</a>
					<ul style="display: none;">
						<li><a href="/sensations/windsurf-kitesurf.php">windsurf / kitesurf</a></li>
						<li><a href="/sensations/lac-lery-poses.php">lac de Léry-Poses</a></li>
						<li><a href="/sensations/grande-paroisse.php">la Grande-Paroisse</a></li>
						<li><a href="/sensations/spots.php">Les spots de l'Ile de France</a></li>
						<li><a href="/sensations/powerkite.php">powerkite</a></li>
					</ul>
				  </li>
                  <li><a href="/emotions/emotions.php">émotions</a></li>
                  <li><a href="/en-chemin/en-chemin.php">en chemin</a>
					<ul style="display: none;">
						<li><a href="/en-chemin/ncis.php">NCIS</a></li>
						
					</ul>	
				  </li>	 				  
                  <li><a href="/reflexions/reflexions.php">?!?</a></li>
                  <li><a href="/informatique/informatique.php">01101</a>
					<ul style="display: none;">
						<li><a href="/informatique/hop.php">hop</a></li>
						<li><a href="/informatique/wescheme.php">wescheme</a></li>
						<li><a href="/informatique/racket.php">racket</a></li>						
					</ul>	
				  </li>					  
                  <li><a href="/science/science.php">e^(i.pi)+1=0</a></li>
                  <li><a href="/a-propos/a-propos.php">à propos...</a></li>
                  <li><a href="/commentaires/commentaires.php">et vous ?</a></li>
               </ul>
            </div>